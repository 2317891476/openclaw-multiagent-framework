...
cat > "$MOCK_BIN_DIR/iflow-quiet-workdir.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
chmod +x "$MOCK_BIN_DIR/iflow-quiet-workdir.sh"

cat > "$MOCK_BIN_DIR/iflow-hard-stall.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo 'MILESTONE: stall-started'
sleep 30
EOF
chmod +x "$MOCK_BIN_DIR/iflow-hard-stall.sh"
...
IFLOW_BIN="$MOCK_BIN_DIR/iflow-quiet-workdir.sh" \
node "$RUNNER" \
  --label smoke-case1 \
  --cwd "$WORKDIR_QUIET" \
...
set +e
IFLOW_BIN="$MOCK_BIN_DIR/iflow-hard-stall.sh" \
node "$RUNNER" \
  --label smoke-case2 \
  --cwd "$WORKDIR_STALL" \
