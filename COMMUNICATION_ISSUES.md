...
| 风险 | 影响 | 缓解 |
|------|------|------|
| ACP Agent 不遵循 prompt 回调指令 | 部分任务无完成通知 | 保留轻量级文件检查作为 fallback |
| before_tool_call hook 尚未完全 wired (Issue #5943) | Plugin 不生效 | 降级为 SOUL.md 约束 + 简化 wrapper |
| sessions_send 到 completion-relay 也 timeout | 通知丢失 | 加重试逻辑（在 prompt 指令中） |
...
```
完整方案:  Plugin hook (自动拦截) + Prompt 注入 (完成回调) + Listener
降级方案:  SOUL.md 规则 + Prompt 注入 (完成回调) + Listener
最小方案:  只做 Prompt 注入 — ACP 完成时用 sessions_send 通知
```

即使只做最小方案（在 ACP prompt 中注入回调指令），也比当前的文件轮询好得多。
