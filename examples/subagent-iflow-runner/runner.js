...
Options:
  ...
  --run-dir <path>         Run directory (default: tmp/iflow-runs/run-<timestamp>)
...
function defaultRunDir(label) {
  ...
  return path.resolve(process.cwd(), 'tmp', 'iflow-runs', name);
}
...
async function main() {
  ...
  const files = {
    ...
    stdout: path.join(runDir, 'iflow.stdout.log'),
    stderr: path.join(runDir, 'iflow.stderr.log'),
