...
3. a local Node.js runner (`runner.js`) that spawns iFlow CLI directly
...
Current machine note: if `iflow` reports OAuth2 expiry, re-authentication is required before real runs will succeed.

## What is included

- `runner.js`: hardened iFlow CLI runner with total-timeout + two-stage stall watchdog
...
- `test_watchdog_smoke.sh`: self-contained smoke test using mock `iflow` binaries
...
```text
RUN_DIR=/abs/path/to/tmp/iflow-runs/run-...
...
Run from any project directory; the run output will go under `./tmp/iflow-runs` unless you override paths.

### Wrapper path

```bash
bash examples/subagent-iflow-runner/run_v1.sh \
...
```bash
SUBAGENT_IFLOW_TIMEOUT_S=3600 \
SUBAGENT_IFLOW_IDLE_TIMEOUT_S=900 \
SUBAGENT_IFLOW_STALL_GRACE_S=300 \
SUBAGENT_IFLOW_KILL_GRACE_MS=5000 \
  bash examples/subagent-iflow-runner/run_v1.sh "Your task here" my-task
```

### Watch a run once

```bash
node examples/subagent-iflow-runner/watcher.js \
  --run-dir ./tmp/iflow-runs/run-2026-03-16T12-34-56-789Z-demo \
  --once
```
...
```bash
bash examples/subagent-iflow-runner/cleanup.sh --keep-days 3 --dry-run
bash examples/subagent-iflow-runner/cleanup.sh --keep-days 3
```
...
```bash
bash examples/subagent-iflow-runner/test_watchdog_smoke.sh
```
...
main agent
  ...
  -> subagent exec: bash examples/subagent-iflow-runner/run_v1.sh "task" "label"
...
- `IFLOW_EXTRA_ARGS` is split on whitespace only; it does not implement shell-grade quoting
